...
$(document).ready(function(e) {
	...
	//密码特殊符号验证
    /*$('body').on('change', '[type=password]', function(event){
        var pwdDom = $(event.currentTarget);
        var pa=/^(?=.{6,16}$)(?![0-9]+$)(?!.*(.).*\1)[0-9a-zA-Z.]+$/;
        if(pa.test(pwdDom.val()) != true){
            $('.ybc_next').attr('disabled', 'disabled');
            $('.denglu').attr('disabled', 'disabled');
            if($('#pwdmsg font').length > 0){
                $('#pwdmsg font').text('密码中不可含有特殊字符');
            }else{
                $('#pwdmsg').text('密码中不可含有特殊字符');
            }

        }else{
            $('.ybc_next').removeAttr('disabled');
