...
class AjaxController extends HomeController
{
	...
	public function top_coin_menu($ajax = 'json')
	{
		...
		$data = (APP_DEBUG ? null : S('movesay_getTopCoinMenu'));
		
		
		$movesay_getCoreConfig = movesay_getCoreConfig();
		if(!$movesay_getCoreConfig){
			$this->error('核心配置有误');
		}
		...
		if (!$data) {
			...
			foreach($movesay_getCoreConfig['movesay_indexcat'] as $k=>$v){
				$data[$k][title] = $v;
			}
...
/* 				if (C('market')[$v['name'] . '_cny']['new_price']) {
	...
	{
		//$data = (APP_DEBUG ? null : S('movesay_allcoin'));
		
		
		$movesay_data=array();
		
		$movesay_data['info']="数据异常";
		$movesay_data['status']=0;
		$movesay_data['url']="";
		...
		if (!$data) {
			...
			$movesay_data['info']="数据正常";
			$movesay_data['status']=1;
			$movesay_data['url']="";
			...
			foreach (C('market') as $k => $v) {
				...
				if($id==1){
					if(stristr($k,'_cny')){
						$movesay_data['url'][$k][0] = $v['title'];
						$movesay_data['url'][$k][1] = round($v['new_price'], $v['round']);
						$movesay_data['url'][$k][2] = round($v['buy_price'], $v['round']);
						$movesay_data['url'][$k][3] = round($v['sell_price'], $v['round']);
						$movesay_data['url'][$k][4] = isset($themarketLogs[$k]) ? $themarketLogs[$k] : 0;//round($v['volume'] * $v['new_price'], 2) * 1;
						$movesay_data['url'][$k][5] = '';
						$movesay_data['url'][$k][6] = round($v['volume'], 2) * 1;
						$movesay_data['url'][$k][7] = round($v['change'], 2);
						$movesay_data['url'][$k][8] = $v['name'];
						$movesay_data['url'][$k][9] = $v['xnbimg'];
						$movesay_data['url'][$k][10] = '';
					}
				}
				if($id==2){
					if(stristr($k,'_btc')){
...
/*         if ($marketLogs) {
	...
	public function getDepth($market = NULL, $trade_moshi = 1, $ajax = 'json')
	{
		...
		if(!$movesay_getCoreConfig){
			...
			$movesay_putong = $movesay_getCoreConfig['movesay_userTradeNum'];
			$movesay_teshu = $movesay_getCoreConfig['movesay_specialUserTradeNum'];
		}
