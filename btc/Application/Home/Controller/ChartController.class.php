...
class ChartController extends HomeController
{
	public function getJsonData($market = NULL, $ajax = 'json')
	{
		if ($market) {
			...
			if(!$movesay_getCoreConfig){
				...
				$movesay_putong = $movesay_getCoreConfig['movesay_userTradeDetailNum'];
				$movesay_teshu = $movesay_getCoreConfig['movesay_specialUserTradeDetailNum'];
			}
			
			$limit = $movesay_putong;
			if(userid()){
	...
	protected function getTradeBuy($market,$limit)
	{
		...
		$mo = M();
		$buy = $mo->query('select id,price,sum(num-deal)as nums from movesay_trade  where status=0 and type=1 and market =\'' . $market . '\' group by price order by price desc limit '.$limit);
		$data = '';
	...
	protected function getTradeSell($market,$limit)
	{
		$limit = intval($limit);
		$mo = M();
		$sell = $mo->query('select id,price,sum(num-deal)as nums from movesay_trade where status=0 and type=2 and market =\'' . $market . '\' group by price order by price asc limit '.$limit);
		$data = '';
	...
	public function getMarketNewKmovesayJson()
	{
		// TODO: SEPARATE
		$input = I('get.');
		$market = (is_array(C('market')[$input['market']]) ? trim($input['market']) : C('market_mr'));
