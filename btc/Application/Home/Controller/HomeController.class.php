	...
	/*	if(WAP_URL !=""){
			...
			if(preg_match('/(iphone|android|Windows\sPhone)/i', $ua)){
				...
				if (isset($_GET['invit'])) {
					...
					if ($user['id']) {
						$movesay_redirect = WAP_URL."/Login/register/invit/".$user['id'];
					}else{
						$movesay_redirect = WAP_URL;
					}
				}else{
					$movesay_redirect = WAP_URL;
				}
				header("Location:".$movesay_redirect);
				die();
			} 
		}
		
		//20170511 movesay 增加获取币类型函数
		
		//C('coin_menu_movesay',array('CNY','BTC','ETH'));*/
