...
class Queue9ce2472db8c3b3d94511365004ce8468Controller extends HomeController
{
	...
	public function upTrade_movesay1111111_8a201aa602cd9448($market = NULL)
	{
		die();
		$userid = rand(1, 2);
		$type = rand(1, 2);
	...
	public function upTrade_movesay_8a201aa602cd9448($market = NULL)
	{
		...
		if ($type == 1) {
			$rs[] = $mo->table('movesay_trade')->add(array('userid' => 0, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => 0, 'type' => 1, 'addtime' => time(), 'status' => 0));
		}
		else if ($type == 2) {
			$rs[] = $mo->table('movesay_trade')->add(array('userid' => 0, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => 0, 'type' => 2, 'addtime' => time(), 'status' => 0));
		}
		else {
			$mo->execute('rollback');
			$mo->execute('unlock tables');
