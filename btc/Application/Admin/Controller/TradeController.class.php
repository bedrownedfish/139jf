...
class TradeController extends AdminController
{
	...
	public function market($field = NULL, $name = NULL)
	{
		...
		foreach($list as $k=>$v){
			...
			$list[$k]['tradetimemovesay'] = $begintradeqq_3479015851."-".$endtradeqq_3479015851;
		}
	...
	public function marketEdit($id = NULL)
	{
		...
		if (empty($_POST)) {
			if (empty($id)) {
				...
				$market_movesay = M('Market')->where(array('id' => $id))->find();
				$this->data = $market_movesay;

				if($market_movesay['begintrade']){
					$beginshi = explode(":",$market_movesay['begintrade'])[0];
					$beginfen = explode(":",$market_movesay['begintrade'])[1];
				}else{
					$beginshi = "00";
					$beginfen = "00";
				}
				
				if($market_movesay['endtrade']){
					$endshi = explode(":",$market_movesay['endtrade'])[0];
					$endfen = explode(":",$market_movesay['endtrade'])[1];
				}else{
					$endshi = "23";
					$endfen = "59";
				}
			...
			$this->assign('movesay_getCoreConfig',$movesay_getCoreConfig['movesay_indexcat']);
