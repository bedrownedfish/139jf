...
class ConfigController extends AdminController
{
	...
	public function coinEdit($id = NULL)
	{
		if (empty($_POST)) {
			...
			$this->assign("movesay_opencoin",$movesay_getCoreConfig['movesay_opencoin']);
			...
			if ($_POST['id']) {
				...
				//对应的商品付款类型增加币种
				$rea = M()->execute('ALTER TABLE  `movesay_shop_coin` ADD  `' . $_POST['name'] . '` VARCHAR(200) NOT NULL');
	...
	public function coinStatus()
	{
		...
		case 'delete':
			...
			foreach ($rs as $k => $v) {
				...
				$rs[] = M()->execute('ALTER TABLE  `movesay_shop_coin` DROP COLUMN ' . $v['name']);
			}
