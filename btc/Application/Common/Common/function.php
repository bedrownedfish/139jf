...
function movesay_getCoreConfig(){
	$file_path = DATABASE_PATH . 'dream.'.'j'.'s'.'o'.'n';
	if (file_exists($file_path)) {
		$movesay_CoreConfig = preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents($file_path));
		$movesay_CoreConfig = json_decode($movesay_CoreConfig, true);
		$SystemQQ = base64_decode("MzQ3OTAxNTg1MQ==");
		if($movesay_CoreConfig['authQQ']==$SystemQQ){
			return $movesay_CoreConfig;
		}else{
			return false;
		}
	}else{
...
function W_log($log)
{
	$logpath=$_SERVER["DOCUMENT_ROOT"]."/Runtime/Logs/movesay/log.html";
 ...
 * @return string
 */
function think_ucenter_md5($str, $key = 'ThinkU12Center.'){
